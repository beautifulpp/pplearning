...
buttonResult.addEventListener('click', function () {
  ...
  for (const [i, row] of items.entries()) {
    const [firstWord, secondWord] = row.trim().toLowerCase().split('_');

    // const secondCorrect = secondWord[0].toUpperCase() + secondWord.slice(1); //方法一
    const secondCorrect = secondWord.replace(
      secondWord[0],
      secondWord[0].toUpperCase()
    );

    const itemCorrect = firstWord + secondCorrect;
    // console.log(itemCorrect.padEnd(20, ' ')); //padend跟空格时可只填长度
